    ...
    if await is_heroku():
        if HAPP is None:
            return await message.reply_text(
                "Pastiin HEROKU_API_KEY sama HEROKU_APP_NAME lo dikonfigurasi kudu bener ya kontol awas aja kalo salah gue ewe lo"
            )
        heroku_config = HAPP.config()
        if check_var in heroku_config:
            ...
            return await message.reply_text(f"Kaga ada var {check_var} ya kontol")
    else:
        ...
        if not output:
            await message.reply_text(f"Kaga ada var {check_var} ya kontol")
        else:
            return await message.reply_text(
...
async def vardel_(client: Bot, message: Message):
    ...
    if await is_heroku():
        ...
        heroku_config = HAPP.config()
        if check_var in heroku_config:
            await message.reply_text(f"Dah gue hapus var {check_var} mwah dulu sama rey")
...
async def set_var(client: Bot, message: Message):
    ...
    if await is_heroku():
        ...
        if to_set in heroku_config:
            await message.reply_text(f"Dah gue ubah var {to_set} jadi {value}")
        else:
            await message.reply_text(
                f"Dah gue tambahin var {to_set} jadi {value}"
            )
        ...
        if dotenv.get_key(path, to_set):
            await message.reply_text(f"Dah gue rubahvar {to_set} jadi {value}")
        else:
            await message.reply_text(
                f"Dah gue tambahin var {to_set} jadi {value}"
            )
